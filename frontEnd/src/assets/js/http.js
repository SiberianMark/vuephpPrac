// router.replace()--v-link内联选项，跟router.go()一样设置路由的跳转，v-link绑定默认是go(),前者不留下历史记录，后者留下历史记录
const apiMethods = {
  methods: {
    apiGet(url, data) {
        ...
        switch (res.code) {
          case 101:
            ...
            if (Cookies.get('rememberPwd')) {
              ...
              setTimeout(() => {
                router.replace('/')// router.replace()--v-link内联选项，跟router.go()一样设置路由的跳转，v-link绑定默认是go(),前者不留下历史记录，后者留下历史记录
              }, 1500)
            }
            break
          case 103:
    ...
    resetCommonData(data) {
      ...
      setTimeout(() => {
        ...
        if (routerUrl != path) {
          router.replace(routerUrl)// router.replace()--v-link内联选项，跟router.go()一样设置路由的跳转，v-link绑定默认是go(),前者不留下历史记录，后者留下历史记录
        } else {
